...
class FileReader:
    ...
    def read(filename):
        try:
            ...
            raise ValueError(f"Файл справочника не найден.")
            return {}
        except json.JSONDecodeError:
            raise ValueError("Ошибка разбора JSON файла")
            return {}
...
class PhoneBook(Contact):
    ...
    def search_contact(self, search_key, search_name):
            ...
            if bool(search_val):
               return search_val
                ...
                raise ValueError(f"Контакт не найден.")
    ...
    def update_contact(self, id, name=None, phone=None, desc=None):
        if id in self.contacts:
            ...
            raise ValueError(f"Контакт для изменения не найден.")

    def delete_contact(self, id):
        if id in self.contacts:
            del self.contacts[id]
            #print(f'Удален контакт с id={id}')
        else:
            raise ValueError(f"Контакт для удаления не найден.")

    def add_contact(self, name, phone, desc):
        new_id = str(max((int(k) for k in self.contacts.keys()),default=0) + 1)
        if self.is_valid_phone(phone):
            ...
            raise ValueError("Неверно введен номер телефона(должен содержать только цифры).")
